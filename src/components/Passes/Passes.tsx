...
export function Passes({
  ...
  useEffect(() => {
    async function loadPassesData() {
      try {
        ...
        const campaign = eligibilityData.referral_code_details?.campaign ?? 'astroncode_guest_pass';
  ...
  const renderTicket = (pass: PassStatus) => {
    ...
    return <Box key={pass.passNumber} flexDirection="column" marginRight={1}>
        ...
        <Text>
          {' ) CC '}
          <Text color="astron">{TEARDROP_ASTERISK}</Text>
          {' ┊ ( '}
        </Text>
        <Text>{'└──────────┘'}</Text>
      </Box>;
  ...
  return <Pane>
      <Box flexDirection="column" gap={1}>
        ...
        <Box flexDirection="column" marginLeft={2}>
          <Text dimColor>
            {referrerReward ? `Share a free week of Astroncode with friends. If they love it and subscribe, you'll get ${formatCreditAmount(referrerReward)} of extra usage to keep building. ` : 'Share a free week of Astroncode with friends. '}
            <Link url={referrerReward ? 'https://docs.astroncode.dev/guest-passes' : 'https://docs.astroncode.dev/guest-passes'}>
              Terms apply.
            </Link>
          </Text>
        </Box>
