 ...
 * - Epoch management: reads worker_epoch from ASTRONCODE_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (ASTRONCODE_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * ASTRONCODE_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    ASTRONCODE_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.ASTRONCODE_WORKER_EPOCH
