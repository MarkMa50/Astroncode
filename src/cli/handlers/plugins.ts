 ...
 * These are dynamically imported only when `astroncode plugin *` or `astroncode plugin marketplace *` runs.
 */
...
export async function pluginValidateHandler(
  ...
  try {
    ...
    // If this is a plugin manifest located inside a .astroncode-plugin directory,
    ...
    if (result.fileType === 'plugin') {
      ...
      if (basename(manifestDir) === '.astroncode-plugin') {
...
export async function pluginListHandler(options: {
  ...
  if (pluginIds.length === 0 && inlinePlugins.length === 0) {
    ...
    if (inlineLoadErrors.length === 0) {
      cliOk(
        'No plugins installed. Use `astroncode plugin install` to install a plugin.',
      )
    }
  }
