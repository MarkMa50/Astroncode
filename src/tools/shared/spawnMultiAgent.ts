...
async function handleSpawnSplitPane(
  ...
  // Build the command to spawn Astroncode with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces ASTRONCODE_* env vars)
  const teammateArgs = [
  ...
  // Includes CLAUDECODE, ASTRONCODE_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new Astroncode instance.
