  ...
  async *submitMessage(
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.ASTRONCODE_EAGER_FLUSH) ||
          isEnvTruthy(process.env.ASTRONCODE_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via ASTRONCODE_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or ASTRONCODE_PLUGIN_SEED_DIR before this runs;
