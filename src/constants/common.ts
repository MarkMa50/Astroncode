...
export function getLocalISODate(): string {
  ...
  if (process.env.ASTRONCODE_OVERRIDE_DATE) {
    return process.env.ASTRONCODE_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.ASTRONCODE_OVERRIDE_DATE
    ? new Date(process.env.ASTRONCODE_OVERRIDE_DATE)
    : new Date()
