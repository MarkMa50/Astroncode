 ...
 * - Checks environment kind (ASTRONCODE_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires ASTRONCODE_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.ASTRONCODE_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but ASTRONCODE_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
