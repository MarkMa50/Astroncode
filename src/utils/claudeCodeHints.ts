/**
 * Astroncode hints protocol.
 *
 * CLIs and SDKs running under Astroncode can emit a self-closing
 * `<astroncode-hint />` tag to stderr (merged into stdout by the shell
 ...
 *
 * See docs/astroncode-hints.md for the vendor-facing spec.
 */
 ...
 */
const HINT_TAG_RE = /^[ \t]*<astroncode-hint\s+([^>]*?)\s*\/>[ \t]*$/gm
...
export function extractClaudeCodeHints(
  ...
  if (!output.includes('<astroncode-hint')) {
