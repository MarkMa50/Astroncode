...
export function getUserAgent(): string {
  ...
  return `claude-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.ASTRONCODE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.ASTRONCODE_ENTRYPOINT) {
    parts.push(process.env.ASTRONCODE_ENTRYPOINT)
  }
  ...
  return `astroncode/${MACRO.VERSION}${suffix}`
}
...
// operators match in robots.txt); the astroncode suffix lets them distinguish
