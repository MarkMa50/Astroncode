/**
 * Perfetto Tracing for Astroncode (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via ASTRONCODE_PERFETTO_TRACE=1 or ASTRONCODE_PERFETTO_TRACE=<path>
 * 2. Optionally set ASTRONCODE_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run Astroncode normally
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.ASTRONCODE_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    // Start periodic full-trace write if ASTRONCODE_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.ASTRONCODE_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
