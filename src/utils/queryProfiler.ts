 ...
 * pipeline from user input to first token arrival. Enable by setting ASTRONCODE_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.ASTRONCODE_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set ASTRONCODE_PROFILE_QUERY=1)'
  }
