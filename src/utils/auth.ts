...
function isManagedOAuthContext(): boolean {
  return (
    isEnvTruthy(process.env.ASTRONCODE_REMOTE) ||
    process.env.ASTRONCODE_ENTRYPOINT === 'claude-desktop'
  )
}
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // local auth-injecting proxy. The launcher sets ASTRONCODE_OAUTH_TOKEN as a
  ...
  if (process.env.ANTHROPIC_UNIX_SOCKET) {
    return !!process.env.ASTRONCODE_OAUTH_TOKEN
  }

  const is3P =
    isEnvTruthy(process.env.ASTRONCODE_USE_BEDROCK) ||
    isEnvTruthy(process.env.ASTRONCODE_USE_VERTEX) ||
    isEnvTruthy(process.env.ASTRONCODE_USE_FOUNDRY)
  ...
  const hasExternalAuthToken =
    ...
    apiKeyHelper ||
    process.env.ASTRONCODE_API_KEY_FILE_DESCRIPTOR
...
export function getAuthTokenSource() {
  ...
  if (process.env.ASTRONCODE_OAUTH_TOKEN) {
    return { source: 'ASTRONCODE_OAUTH_TOKEN' as const, hasToken: true }
  }
  ...
  if (oauthTokenFromFd) {
    ...
    if (process.env.ASTRONCODE_OAUTH_TOKEN_FILE_DESCRIPTOR) {
      return {
        source: 'ASTRONCODE_OAUTH_TOKEN_FILE_DESCRIPTOR' as const,
        hasToken: true,
      }
    }
    return {
...
export function getAnthropicApiKeyWithSource(
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      !apiKeyEnv &&
      !process.env.ASTRONCODE_OAUTH_TOKEN &&
      !process.env.ASTRONCODE_OAUTH_TOKEN_FILE_DESCRIPTOR
    ) {
      throw new Error(
        'ANTHROPIC_API_KEY or ASTRONCODE_OAUTH_TOKEN env var is required',
      )
    }
 ...
 * Uses ASTRONCODE_API_KEY_HELPER_TTL_MS env var if set and valid,
 * otherwise defaults to 5 minutes
 */
export function calculateApiKeyHelperTTL(): number {
  const envTtl = process.env.ASTRONCODE_API_KEY_HELPER_TTL_MS
  ...
  if (envTtl) {
    ...
    logForDebugging(
      `Found ASTRONCODE_API_KEY_HELPER_TTL_MS env var, but it was not a valid number. Got ${envTtl}`,
      { level: 'error' },
    )
  }
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  if (process.env.ASTRONCODE_OAUTH_TOKEN) {
    ...
    return {
      accessToken: process.env.ASTRONCODE_OAUTH_TOKEN,
      refreshToken: null,
      expiresAt: null,
...
export function getOtelHeadersFromHelper(): Record<string, string> {
  ...
  const debounceMs = parseInt(
    process.env.ASTRONCODE_OTEL_HEADERS_HELPER_DEBOUNCE_MS ||
...
export function getAccountInformation() {
  ...
  if (
    authTokenSource === 'ASTRONCODE_OAUTH_TOKEN' ||
    authTokenSource === 'ASTRONCODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ) {
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  const isEnvVarToken =
    source === 'ASTRONCODE_OAUTH_TOKEN' ||
    source === 'ASTRONCODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ...
  if (!profile) {
    ...
    return {
      ...
      message:
        ...
        `verification (tokens from 'atroncode setup-token' do not include this scope).\n` +
        `Try again, or obtain a full-scope token via 'atroncode auth login'.`,
    }
  }
  ...
  if (isEnvVarToken) {
    const envVarName =
      source === 'ASTRONCODE_OAUTH_TOKEN'
        ? 'ASTRONCODE_OAUTH_TOKEN'
        : 'ASTRONCODE_OAUTH_TOKEN_FILE_DESCRIPTOR'
    return {
      valid: false,
      message:
  ...
  return {
    ...
    message:
      ...
      `Please log in with the correct organization: atroncode auth login`,
  }
}
