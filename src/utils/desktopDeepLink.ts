...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'Atroncode Desktop is not installed. Install the local companion app before using /desktop.',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open Atroncode Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
