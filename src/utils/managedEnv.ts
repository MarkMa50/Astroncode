 ...
 * ASTRONCODE_PROVIDER_MANAGED_BY_HOST in spawn env), strip
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.ASTRONCODE_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // flagSettings env applied. Eligibility reads ASTRONCODE_USE_BEDROCK,
  ...
  // when ASTRONCODE_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * ASTRONCODE_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
