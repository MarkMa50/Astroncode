...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error('Astroncode web sessions require authentication with a Claude.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
...
export async function teleportToRemote(options: {
   ...
   * environmentId is set, ASTRONCODE_OAUTH_TOKEN is auto-injected from the
  ...
  try {
    ...
    if (options.environmentId) {
      ...
      const envVars = {
        ASTRONCODE_OAUTH_TOKEN: accessToken,
    ...
    if (!gitSource && bundleSeedGateOn) {
      ...
      if (!bundle.success) {
        ...
        const setup = repoInfo ? '. Please setup GitHub on https://astron.dev/code' : '';
        let msg: string;
