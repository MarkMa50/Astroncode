...
function getEntrypoint(): string | undefined {
  return process.env.ASTRONCODE_ENTRYPOINT
}
...
class Project {
   ...
   * ASTRONCODE_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.ASTRONCODE_SKIP_PROMPT_HISTORY)
    )
  }
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.ASTRONCODE_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // ASTRONCODE_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.ASTRONCODE_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.ASTRONCODE_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
