...
/**
 * Generates Astroncode specific snapshot content
...
async function getSnapshotScript(
  ...
  // Generate the user content and Astroncode content
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with Astroncode defaults only`,
        )
      }
      ...
      execFile(
        ...
        {
          env: {
            ...((process.env.ASTRONCODE_DONT_INHERIT_ENV
              ? {}
