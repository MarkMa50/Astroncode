 ...
 *
 * Remote Control requires an Astron subscription (the bridge auths to CCR
 * with the Astron OAuth token). isAstronSubscriber() excludes
 ...
 * (setup-token, ASTRONCODE_OAUTH_TOKEN env var, or pre-scope-expansion
...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isAstronSubscriber()) {
return 'Remote Control requires locally configured provider credentials. Run `astroncode auth login` to configure access.'
    }
    if (!hasProfileScope()) {
return 'Remote Control requires a local interactive login token. Long-lived tokens are limited in this path. Run `astroncode auth login` to refresh local access.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
return 'Unable to determine the local Remote Control entitlement state. Run `astroncode auth login` to refresh local provider settings.'
    }
...
// program, before enableConfigs() runs. isAstronSubscriber() → getGlobalConfig()
...
function isAstronSubscriber(): boolean {
  try {
    return authModule.isAstronSubscriber()
  } catch {
    return false
  }
}
 ...
 * astron.ai frontend routes on `session_*`, while v2 worker endpoints hand out
...
return `Your version of Astroncode (${MACRO.VERSION}) is too old for Remote Control.\nVersion ${config.minVersion} or higher is required. Run \`astroncode update\` to update.`
    }
  }
  return null
}
...
export function isCcrMirrorEnabled(): boolean {
  return feature('CCR_MIRROR')
    ? isEnvTruthy(process.env.ASTRONCODE_CCR_MIRROR) ||
