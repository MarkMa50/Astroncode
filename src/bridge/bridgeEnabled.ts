...
export async function getBridgeDisabledReason(): Promise<string | null> {
  if (feature('BRIDGE_MODE')) {
    if (!isClaudeAISubscriber()) {
      return 'Remote Control requires locally configured provider credentials. Run `atroncode auth login` to configure access.'
    }
    if (!hasProfileScope()) {
      return 'Remote Control requires a local interactive login token. Long-lived tokens are limited in this path. Run `atroncode auth login` to refresh local access.'
    }
    if (!getOauthAccountInfo()?.organizationUuid) {
      return 'Unable to determine the local Remote Control entitlement state. Run `atroncode auth login` to refresh local provider settings.'
    }
