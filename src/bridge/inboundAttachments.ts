 ...
 * (oauth-authed, same store), write to ~/.astroncode/uploads/{sessionId}/, and
 * return @path refs to prepend. Astroncode's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches Astroncode, just without @path.
 */
...
import { getAstronConfigHomeDir } from '../utils/envUtils.js'
...
function uploadsDir(): string {
  return join(getAstronConfigHomeDir(), 'uploads', getSessionId())
}
...
async function resolveOne(att: InboundAttachment): Promise<string | undefined> {
  ...
  try {
    ...
    // ASTRONCODE_CUSTOM_OAUTH_URL — keep it inside the try so a bad
