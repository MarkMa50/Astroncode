...
/** Full error printed when `astroncode remote-control` is run without auth. */
...
export type WorkSecret = {
  ...
  astroncode_args?: Record<string, string> | null
...
/**
 * How `astroncode remote-control` chooses session working directories.
 ...
 * `metadata.worker_type` at environment registration so astron.ai can filter
 ...
 */
export type BridgeWorkerType = 'astroncode' | 'astroncode_assistant'
...
export type BridgeConfig = {
   ...
   * instead of creating a new one. Used by `astroncode remote-control
