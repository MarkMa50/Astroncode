 ...
 *
 * Used by both `astroncode remote-control` (empty session so the user has somewhere to
...
export async function createBridgeSession({
  ...
  const { getAstronOAuthTokens } = await import('../utils/auth.js')
  ...
  const accessToken =
    getAccessToken?.() ?? getAstronOAuthTokens()?.accessToken
  if (!accessToken) {
    ...
    if (parsed) {
      ...
      gitOutcome = {
        ...
        git_info: {
          type: 'github',
          repo: `${owner}/${name}`,
          branches: [`astroncode/${branch || 'task'}`],
        },
      }
    } else {
...
export async function getBridgeSession(
  ...
  const accessToken =
    opts?.getAccessToken?.() ?? getAstronOAuthTokens()?.accessToken
  if (!accessToken) {
 ...
 * explicit client action. Both `astroncode remote-control` (standalone bridge) and the
 ...
 * connection is active, so the title stays in sync on astron.ai/code.
 *
