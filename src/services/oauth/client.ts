...
export async function refreshOAuthToken(
  ...
  try {
    ...
    // ASTRONCODE_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.ASTRONCODE_ACCOUNT_UUID
  const envUserEmail = process.env.ASTRONCODE_USER_EMAIL
  const envOrganizationUuid = process.env.ASTRONCODE_ORGANIZATION_UUID
