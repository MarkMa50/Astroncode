...
export async function call(onDone: LocalJSXCommandOnDone, context: LocalJSXCommandContext): Promise<React.ReactNode | null> {
  try {
    ...
    if (isClaudeAISubscriber()) {
      ...
      if (isMax20x) {
        setTimeout(onDone, 0, 'Local provider credentials are already configured. Run /login if you want to switch accounts, models, or billing settings.');
        return null;
      }
    }
    ...
    setTimeout(onDone, 0, 'Failed to open browser. Re-run /login to update your local provider settings.');
  }
  return null;
}
