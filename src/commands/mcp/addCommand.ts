...
export function registerMcpAddCommand(mcp: Command): void {
  mcp
    ...
    .description(
      'Add an MCP server to Atroncode.\n\n' +
        'Examples:\n' +
        '  # Add HTTP server:\n' +
        '  atroncode mcp add --transport http sentry https://mcp.sentry.dev/mcp\n\n' +
        '  # Add HTTP server with headers:\n' +
        '  atroncode mcp add --transport http corridor https://app.corridor.dev/api/mcp --header "Authorization: Bearer ..."\n\n' +
        '  # Add stdio server with environment variables:\n' +
        '  atroncode mcp add -e API_KEY=xxx my-server -- npx my-mcp-server\n\n' +
        '  # Add stdio server with subprocess flags:\n' +
        '  atroncode mcp add my-server -- my-command --some-flag arg1',
    )
    .option(
      '-s, --scope <scope>',
    ...
    .addOption(
      new Option(
        '--xaa',
        "Enable XAA (SEP-990) for this server. Requires 'atroncode mcp xaa setup' first. Also requires --client-id and --client-secret (for the MCP server's AS).",
    ...
    .action(async (name, commandOrUrl, args, options) => {
      ...
      try {
        ...
        if (options.xaa && !isXaaEnabled()) {
          cliError(
            'Error: --xaa requires ASTRONCODE_ENABLE_XAA=1 in your environment',
          )
        }
        ...
        if (xaa) {
          ...
          if (!getXaaIdpSettings()) {
            missing.push(
              "'atroncode mcp xaa setup' (settings.xaaIdp not configured)",
            )
          }
          if (missing.length) {
