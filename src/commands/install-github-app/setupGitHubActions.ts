...
async function createWorkflowFile(
  ...
  if (secretName === 'ASTRONCODE_OAUTH_TOKEN') {
    // For OAuth tokens, use the astroncode_oauth_token parameter
    content = workflowContent.replace(
      /anthropic_api_key: \$\{\{ secrets\.ANTHROPIC_API_KEY \}\}/g,
      `astroncode_oauth_token: \${{ secrets.ASTRONCODE_OAUTH_TOKEN }}`,
    )
  ...
  if (createFileResult.code !== 0) {
    if (
      ...
      throw new Error(
        `Failed to create workflow file ${workflowPath}: An Atroncode workflow file already exists in this repository. Please remove it first or update it manually.`,
      )
    }
    ...
    const helpText =
      ...
      '· For manual setup → Review your local Atroncode workflow template'
  ...
  try {
    ...
    if (!skipWorkflow) {
      ...
      if (selectedWorkflows.includes('claude-review')) {
        workflows.push({
          path: '.github/workflows/astroncode-review.yml',
          content: CODE_REVIEW_PLUGIN_WORKFLOW_CONTENT,
          message: 'Atroncode Review workflow',
        })
      }
