...
throw new Error('Could not install - another process is currently installing Astroncode. Please try again in a moment.');
        }
...
setTimeout(onDone, 2000, 'Astroncode installation completed successfully', {
        display: 'system' as const
      });
    } else if (state.type === 'error') {
      // Give error message time to render before exiting
setTimeout(onDone, 3000, 'Astroncode installation failed', {
        display: 'system' as const
      });
    }
  }, [state, onDone]);
  return <Box flexDirection="column" marginTop={1}>
      {state.type === 'checking' && <Text color="astron">Checking installation status...</Text>}

      {state.type === 'cleaning-npm' && <Text color="warning">Cleaning up old npm installations...</Text>}

      {state.type === 'installing' && <Text color="astron">
          Installing Astroncode native build {state.version}...
        </Text>}

      {state.type === 'setting-up' && <Text color="astron">Setting up launcher and shell integration...</Text>}
      ...
      {state.type === 'success' && <Box flexDirection="column" gap={1}>
          <Box>
            ...
            <Text color="success" bold>
              Astroncode successfully installed!
            </Text>
          </Box>
          <Box marginLeft={2} flexDirection="column" gap={1}>
            {state.version !== 'current' && <Box>
                <Text dimColor>Version: </Text>
                <Text color="astron">{state.version}</Text>
              </Box>}
            <Box>
...
export const install = {
  ...
  name: 'install',
  description: 'Install Astroncode native build',
